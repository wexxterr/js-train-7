...
function compareSymbolsForKey(symbol1, symbol2) {
  ...
  let comparesym = symbol1 === symbol2
  return comparesym;
}
...
function getSymbolKey(symbol) {
  ...
  let getKey = Symbol.keyFor(symbol);
  return getKey
}
...
let myObject = {
  ...
  [Symbol.iterator]() {
    ...
    return {
      next: () => {
        if (currentIndex < this.category.length) {
          let value = this.category[currentIndex].name;
          currentIndex++;
